<div class="row">
    @if(isset($patrimonio))
        <input type="hidden" name="cargo_id" value="{{$patrimonio->id}}">
    @endif
    <div class="col-6">
    ...
    <div class="col-6">
        ...
        <input class="form-control @error('descricao') is-invalid @enderror" id="descricao" type="text"
               name="descricao" @if(isset($patrimonio)) value="{{$patrimonio->descricao}}"
        ...
        <select class="form-control" name="classificacao_id" required>
            ...
            @foreach($classificacoes as $classificacao)
                <option value="{{$classificacao->id}}"
                        @if(isset($patrimonio) && $patrimonio->classificacao->id == $classificacao->id || old('classificacao_id') == $classificacao->id) selected @endif>{{$classificacao->nome}}</option>
            @endforeach
        </select>
    </div>
        ...
        <select class="form-control" name="origem_id" required>
            ...
            @foreach($origens as $origem)
                <option value="{{$origem->id}}"
                        @if(isset($patrimonio) && $patrimonio->origem->id == $origem->id || old('origem_id') == $origem->id) selected @endif>{{$origem->nome}}</option>
            @endforeach
        </select>
    </div>
        ...
        <select class="form-control" name="situacao_id" required>
            ...
            @foreach($situacoes as $situacao)
                <option value="{{$situacao->id}}"
                        @if(isset($patrimonio) && $patrimonio->situacao->id == $situacao->id || old('situacao_id') == $situacao->id) selected @endif>{{$situacao->nome}}</option>
            @endforeach
        </select>
    </div>
</div>
        ...
        <select class="form-control" id="predio_select" name="predio_id" required>
            ...
            @foreach($predios as $predio)
                <option value="{{$predio->id}}"
                        @if(isset($patrimonio) && $patrimonio->sala->predio->id == $predio->id || old('predio_id') == $predio->id) selected @endif>{{$predio->nome}}</option>
            @endforeach
        </select>
    </div>
...
<div class="row mt-4">
        ...
        <select class="form-control" name="servidor_id" required>
            ...
            @foreach($servidores as $servidor)
                <option value="{{$servidor->id}}"
                        @if(isset($patrimonio) && $patrimonio->servidor->id == $servidor->id || old('servidor_id') == $servidor->id) selected @endif>{{$servidor->user->name}}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="col-4">
        ...
        <input class="form-control" type="number" id="valor" name="valor" step="0.01" min="0.01" placeholder="0.00">         
    </div>
