<div class="row">
    @if(isset($movimento))
        <input type="hidden" name="cargo_id" value="{{$movimento->id}}">
    @endif
    <div class="col-6">
        ...
        <input class="form-control @error('nome') is-invalid @enderror" id="nome" type="text"
               name="nome" @if(isset($movimento)) value="{{$movimento->nome}}"
    ...
    <div class="col-6">
        ...
        <input class="form-control @error('descricao') is-invalid @enderror" id="descricao" type="text"
               name="descricao" @if(isset($movimento)) value="{{$movimento->descricao}}"
               @else value="{{old('descricao')}}" @endif required
        ...
        <select class="form-control" name="classificacao_id" required>
            ...
            @foreach($classificacoes as $classificacao)
                <option value="{{$classificacao->id}}"
                        @if(isset($movimento) && $movimento->classificacao->id == $classificacao->id || old('classificacao_id') == $classificacao->id) selected @endif>{{$classificacao->nome}}</option>
            @endforeach
        </select>
    </div>
        ...
        <select class="form-control" name="origem_id" required>
            ...
            @foreach($origens as $origem)
                <option value="{{$origem->id}}"
                        @if(isset($movimento) && $movimento->origem->id == $origem->id || old('origem_id') == $origem->id) selected @endif>{{$origem->nome}}</option>
            @endforeach
        </select>
    </div>
        ...
        <select class="form-control" name="situacao_id" required>
            ...
            @foreach($situacoes as $situacao)
                <option value="{{$situacao->id}}"
                        @if(isset($movimento) && $movimento->situacao->id == $situacao->id || old('situacao_id') == $situacao->id) selected @endif>{{$situacao->nome}}</option>
            @endforeach
        </select>
    </div>
</div>
        ...
        <select class="form-control" id="predio_select" name="predio_id" required>
            ...
            @foreach($predios as $predio)
                <option value="{{$predio->id}}"
                        @if(isset($movimento) && $movimento->sala->predio->id == $predio->id || old('predio_id') == $predio->id) selected @endif>{{$predio->nome}}</option>
            @endforeach
        </select>
    </div>
...
<div class="row mt-4">
        ...
        <select class="form-control" name="servidor_id" required>
            ...
            @foreach($servidores as $servidor)
                <option value="{{$servidor->id}}"
                        @if(isset($movimento) && $movimento->servidor->id == $servidor->id || old('servidor_id') == $servidor->id) selected @endif>{{$servidor->user->name}}</option>
            @endforeach
        </select>
    </div>
        ...
        <div class="row">
            ...
            <div class="col-6">
                <input class="form-check-input" type="radio" name="nota_fiscal_radio" id="nota_fiscal_sim"
                       @if(isset($movimento) && $movimento->nota_fiscal != null) checked @endif>
            ...
            <div class="col-6">
                <input class="form-check-input" type="radio" name="nota_fiscal_radio" id="nota_fiscal_nao"
                       @if(isset($movimento) && $movimento->nota_fiscal == null || !isset($movimento)) checked
                       @endif value="false">
    ...
    <div class="col-5" id="anexo_nota_fiscal" style="display: none;">
        ...
        <input class="form-control @error('nota_fiscal') is-invalid @enderror"
               ...
               @if(isset($movimento) && $movimento->nota_fiscal != null) style="width: 135px" @endif >
        @error('nota_fiscal')
        <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
        @enderror
        @if(isset($movimento) && $movimento->nota_fiscal != null)
            <span
...
<script>
    $("#predio_select").on('change', function () {
        ...
        $(document).ready(function () {
            @if(isset($movimento->nota_fiscal) && $movimento->nota_fiscal != null)
        ...
        $("#nota_fiscal_sim").click(function () {
            ...
            @if(isset($movimento->nota_fiscal) && $movimento->nota_fiscal->nota_fiscal == null)
